...
def call_IPVN(path_0):
    ...
    """
    IPVN = pd.read_excel(path_0+"/Data/IPVN_ind_mpi_estr_IVtrim20_indice.xls",skiprows=range(0,8),usecols="A:K").dropna(axis=0,how="all")
    IPVN = IPVN.iloc[1:-4]
    Y = IPVN["Año"].dropna()
...
def cartera_vivienda(path_0):
    ...
    """
    c_vivienda = pd.read_excel(path_0+"/Data/carteradevivienda.xlsx",sheet_name="CONSOLIDADO",usecols="A:BO",skiprows=range(0,4))
    data = np.array(c_vivienda.iloc[:,-2] + c_vivienda.iloc[:,-1])
    ind = c_vivienda.iloc[:,0]
...
def tasa(path_0):
    file = pd.read_excel(path_0+"/Data/carteradevivienda.xlsx", None);
    ...
    dfs = []
    for n  in name:
        t_vivienda = pd.read_excel(path_0+"/Data/carteradevivienda.xlsx",sheet_name=n,usecols="A:AI",skiprows=range(0,5))
...
def call_inflacion(path_0):
    """
    link: https://totoro.banrep.gov.co/estadisticas-economicas/faces/pages/charts/line.xhtml?facesRedirect=true
    """
    inf = pd.read_csv(path_0 + "/Data/inflacion.csv",parse_dates=True,index_col=["Fecha"])/100
    inf.columns = ["Inflacion"]
    inf.index = pd.to_datetime(inf.index,yearfirst=True,format="%Y/%m").to_period("D")
    return inf

def call_banrep(path_0):
    """
    Link: https://totoro.banrep.gov.co/estadisticas-economicas/faces/pages/charts/line.xhtml?facesRedirect=true
    """
    Brep = pd.read_csv(path_0 + "/Data/tasa_banrep.csv",parse_dates=True,index_col=["Fecha"])/100
...
def call_PIB(path_0) :
    ...
    """  
    PIB= pd.read_excel(path_0+"/Data/Anexos_produccion_constantes_IV_2020.xlsx",sheet_name="Cuadro 1", skiprows=range(0,76),usecols="C:BO") 
